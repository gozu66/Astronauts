...
public class BoidTest1 : MonoBehaviour {public float minSpeed = 20.0f;
	...
	void Start () {
		...
		otherFlocks = new BoidTest1[tempFlocks.Length];
		for (int i = 0;i<tempFlocks.Length;i++) {
			objects[i] = tempFlocks[i].transform;
			otherFlocks[i] = (BoidTest1)tempFlocks[i];
		}
		//Null Parent as the flock leader will be
		//BoidTest1Controller object
		...
		for (int i = 0;i<objects.Length;i++){
			...
			if (transform != transformComponent) {
				...
				if (d < followRadius) {
					...
					f = d / followRadius;
					BoidTest1 otherSealgull = otherFlocks[i];
